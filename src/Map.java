public class Map{
	public static short[][] map; //맵의 배열 1일 때 흑, -1일 때 백, 0일 때 돌이 안놓여짐
	private final short BLACK=1; 
	private final short WHITE=-1;
	public static boolean checkBNW=true; //흑차례 백차례 확인
	...
	public boolean winCheck(int x,int y){
		...
		|| winCheckCenterLUT(x, y)||winCheckCenterRLT(x, y)||winCheckCenterRUT(x, y)||winCheckCenterUDT(x, y))
	...
	//1 4  ->1 5 
	//센터 업다운
	public boolean winCheckCenterUDT(int x,int y) {
		try{
	...
	//센터 라이트 레프트
	public boolean winCheckCenterRLT(int x,int y) {
		try {
	...
	//센터 라이트 대각선
	public boolean winCheckCenterRUT(int x,int y) {
		try {
			for(int i=y+3, z=x-3;i>y-3;i--,z++){
				if(map[y][x]!=map[i][z])
					return false;
			}
		} catch (ArrayIndexOutOfBoundsException e) {
	...
	public boolean winCheckCenterLUT(int x,int y) {
		try {
			for(int i=y+3, z=x+3;i>y-3;i--,z--){
				if(map[y][x]!=map[i][z])
					return false;
